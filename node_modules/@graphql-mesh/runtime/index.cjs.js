...
function applyResolversHooksToSchema(schema$1, pubsub) {
    ...
    return schema.addResolversToSchema({
        schema: schema$1,
        resolvers: applyResolversHooksToResolvers(schema$1, sourceResolvers, pubsub),
...
async function getMesh(options) {
    ...
    await Promise.all(options.sources.map(async (apiSource) => {
        ...
        const { wrapTransforms, noWrapTransforms } = utils$1.groupTransforms(apiSource.transforms);
    ...
    async function meshExecute(document, variables, context, rootValue) {
        ...
        return graphql.execute({
            document: utils$1.ensureDocumentNode(document),
            contextValue,
            rootValue: rootValue || {},
